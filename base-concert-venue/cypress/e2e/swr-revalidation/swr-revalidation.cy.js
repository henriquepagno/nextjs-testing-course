...
it("should refresh the shows page after 30 seconds", () => {
  ...
  // advance time 30 seconds more; now additional sold out show should display
...
it("should refresh the reservations page after fifteen seconds", () => {
  ...
  // (in an app where user/password weren't filled, would also need to get from env vars and fill)
  ...
  cy.tick(ONE_SECOND);
  cy.findByText(/10 seats left/).should("exist");

  // advance time and check again
  cy.tick(FIFTEEN_SECONDS);
  cy.findByText(/8 seats left/).should("exist");
});
